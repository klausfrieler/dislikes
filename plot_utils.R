...
mean_value_plot2 <- function(data, degree = "strong", max_val = 7, weighted = F, scale = T){
  y_intercept <- 3
  #browser()
  #data <- data %>% filter(type == !!type)
  if(scale || weighted){
  ...
  q <- q + guides(size = "none")
  q <- q + guides(shape = "none")
  q <- q + labs(x = "", y = x_label, title = sprintf("Degree: %s %s", str_to_title(degree), ifelse(weighted, "(weighted)", "")))
  q <- q + scale_color_manual(values = c("black", "red"), name = "")#stringr::str_to_title(type))
  q
}
